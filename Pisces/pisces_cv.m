...
n_idx = randsample(N*N,N*N);%一列1-N×N的乱序列
Atrain = zeros(N,N,T,Kfold);%T*Kfold个N×N矩阵
Atrain2 = Atrain;
for t = 1:T
    ...
    for k = 1:Kfold
        at = A(:,:,t);
        cvidxtemp = zeros(N,N);
        test = n_idx((k-1)*floor(N*N/Kfold)+1: floor(N*N/Kfold)*k);%选择第k组的所有元素标号
        at(test) = 0;%把所有第k组的元素都变为0
        cvidxtemp(test) = 1;%把测试集变为1
        cvidxtemp = triu(cvidxtemp) + triu(cvidxtemp)';%取对称的节点对
        cvidx(:,:,k) = cvidxtemp;
        at = triu(at) + triu(at)';%训练集也取对称矩阵，节点对
        Atrain(:,:,t,k) = at;
